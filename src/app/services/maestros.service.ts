...
export class MaestrosService {
  ...
  // Obtener lista de maestros con paginación y filtros
  public obtenerListaMaestros(params?: any): Observable<any> {
    const token = this.facadeService.getSessionToken();
    let headers: HttpHeaders;
    if (token) {
      headers = new HttpHeaders({ 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token });
    ...
    return this.http.get<any>(`${environment.url_api}/lista-maestros/`, { headers, params: httpParams });
  }

  // En maestros.service.ts, verifica que tengas este método:
  public obtenerMaestroPorId(id: number): Observable<any>{
    ...
    return this.http.get<any>(`${environment.url_api}/maestros/?id=${id}`, { headers });
  }
